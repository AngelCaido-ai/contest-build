...
### backend
- **Fix:** Удалён некорректный переход `TERMS_LOCKED → CREATIVE_DRAFT` из карты `ALLOWED_TRANSITIONS`. Этот переход позволял (теоретически) начать работу над креативом, минуя эскроу-оплату, что противоречит требованиям escrow-flow. На практике UI бота, miniapp и бэкенд (`do_create_creative`, `ROLE_ALLOWED_STATUSES`) блокировали этот путь, но карта переходов была неконсистентна.
