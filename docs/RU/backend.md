...
| Метод | Путь | Описание | Авторизация |
|---|---|---|---|
| `POST` | `/deals/` | Создать сделку (из листинга или заявки); 409 с `deal_id` если уже есть активная сделка по листингу или по заявке+каналу. Для листингов цена берётся из `price_usd` (приоритет) или `price_ton` (fallback), пользователь не может задать свою цену | JWT |
...
| `0008_datetime_timezone_aware` | Все DateTime-колонки переведены на `TIMESTAMP WITH TIME ZONE` для корректной сериализации дат с таймзоной |
