...
- [Inline-календарь](#inline-календарь-keyboardscalendarpy)
- [API-клиент](#api-клиент-api_clientpy)
...
Поддерживает deep link параметр `deal_{id}` (формат: `/start deal_123`). При наличии параметра активирует указанную сделку в чате — аналогично команде `/deal DEAL_ID`. Используется кнопкой «Перейти в бота» в Mini App.
...
## Inline-календарь (`keyboards/calendar.py`)

Виджет для удобного выбора даты и времени публикации через inline-клавиатуру. Заменяет ручной ввод ISO-строк.

### Поток

1. **Календарь** — сетка дней текущего месяца с навигацией ◀/▶. Прошедшие дни неактивны. Сегодняшний день выделен `[N]`.
2. **Часы** — кнопки 00–23 (4 ряда по 6). Кнопка «◀ Назад» возвращает к календарю.
3. **Минуты** — кнопки с шагом 5 минут (:00, :05, …, :55). Кнопка «◀ Назад» возвращает к часам.
4. **Результат** — формируется ISO-строка `YYYY-MM-DDTHH:MM:00+03:00` (МСК).

### Callback-префиксы

| Префикс    | Назначение                |
|------------|---------------------------|
| `dtp:nav:` | Навигация по месяцам      |
| `dtp:day:` | Выбор дня                 |
| `dtp:hr:`  | Выбор часа                |
| `dtp:mn:`  | Выбор минут (финальный)   |
| `dtp:ign`  | Неактивная кнопка (заглушка) |
| `dtp:skip` | Пропустить (для опциональных полей) |

### Контексты использования

| Контекст          | FSM-состояние                  | Обязательно | Следующий шаг                          |
|-------------------|--------------------------------|-------------|----------------------------------------|
| `terms`           | `DealTermsState.publish_at`    | Да          | → `verification_window`                |
| `publish_at`      | `DealPublishAtState.publish_at`| Да          | → обновление publish_at, карточка сделки |
| `creative_status` | `CreativeStatusState.publish_at`| Да         | → одобрение креатива, карточка сделки   |
| `listing_respond` | `ListingRespondState.publish_at`| Нет (skip) | → `creative`                           |

### Текстовый fallback

Если пользователь вводит текст вместо использования календаря, принимаются форматы:
- `15.02 18:30` — дата без года (текущий год, МСК)
- `15.02.2026 18:30` — дата с годом (МСК)
